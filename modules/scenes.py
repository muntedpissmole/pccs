...
def activate_scene(
    ...
    """Activate a lighting scene using main_config for ramp time."""
    
    scene_config = get_scene_config(scene_name)   # ← Renamed for clarity
    if not scene_config:
    ...
    scene_display_name = scene_config.get("name", scene_name.title())

    try:
        # Get ramp time from the MAIN config (INI file)
        ramp_ms = main_config.getint('lighting', 'scene_ramp_time_ms', fallback=1500)

        if scene_config.get("all_off", False):
        ...
        for light_name, value in scene_config["lights"].items():
