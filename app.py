...
def validate_static_config(config):
    ...
    if 'screens' in config:
        ...
        for name, sconf in screens.items():
            ...
            if 'enabled' in sconf and not isinstance(sconf['enabled'], bool):
                raise ValueError(f"'enabled' for screen '{name}' must be true or false")
    ...
    if system_toast_sec is not None:
        if not isinstance(system_toast_sec, int) or system_toast_sec <= 0:
            raise ValueError("system_toast_display_time must be a positive integer")
    rules_toast_sec = config.get('rules_toast_display_time')
    if rules_toast_sec is not None:
        if not isinstance(rules_toast_sec, int) or rules_toast_sec <= 0:
            raise ValueError("rules_toast_display_time must be a positive integer")
...
# Only track power state for enabled screens
current_screen_levels = {name: 'off' for name, conf in screens_config.items() if conf.get('enabled', True)}
screen_sids = {}
states = {}
...
def handle_connect():
    ...
    is_screen = any(remote_addr == conf['ip'] and conf.get('enabled', True) for conf in screens_config.values())
    screen_name = None
    if is_screen:
        screen_name = next((name for name, conf in screens_config.items() if remote_addr == conf['ip'] and conf.get('enabled', True)), None)
        if screen_name:
...
rules_toast_display_time_sec = static_config.get('rules_toast_display_time', 10)
...
def handle_shutdown_system():
    ...
    for screen_name, conf in screens_config.items():
        if not conf.get('enabled', True):
            logger.info(f"Skipping disabled screen during shutdown: {screen_name}")
            continue
        ip = conf.get('ip')
        username = conf.get('username', 'pi')
        if not ip:
        ...
        if result == 0:
            logger.info(f"Shutdown command sent successfully to {screen_name}")
            successful.append(screen_name)
        else:
            exit_code = result >> 8
            logger.error(f"Failed to send shutdown to {screen_name} – exit code {exit_code}")
